...
#ifndef DesviaTestApp_H
#define DesviaTestApp_H
 ...
 */
namespace Veins {
class DesviaTestApp : public BaseWaveApplLayer {
private:
        double startTime;
	...
	public:
		...
		simtime_t currentLaneExpectedTravelTime=0.0f;
		...
		int delayFactorThreshold = 2;
		bool reroutingByProbabilityEvaluated = false;
...
private:
    ...
    std::map<std::string, double> GetRoadsLengths();
    double GetRoadLength(std::string roadId);
/*    std::map<std::string, double> GetRoadsTravelTimes();
    double GetRoadTravelTime(std::string roadId);*/
};
}
