...
void TraCITestApp::initialize(int stage) {
	...
	if (stage == 0) {
	    ...
	    if (debug){
	        ...
	        for(std::string s : mobility->getCommandInterface()->getJunctionIds()){
	            std::cout << "Bruno - Junction Ids <" << s << "> position: " <<  mobility->getCommandInterface()->junction(s).getPosition() << std::endl;
	        }
	    }
...
void TraCITestApp::handlePositionUpdate() {
	...
	if (t > 10){
	    if (t > 22){
	        traciVehicle->changeRoute("27", 9999);
	    }
	    traciVehicle->changeRoute("28", 9999);
	}


/*	if ((t >= 19 && t <= 22)){
	    std::cout << "Vehicle get Road Id" << mobility->getRoadId() << std::endl;
	    int xx=0;
	    for(std::string s : traciVehicle->getPlannedRoadIds()){
	        std::cout << "Vehicle getCurrentRoadPlannedRoadIds  " << xx++ << " " << s << std::endl;
	    }

	    //std::cout << "Vehicle getCurrentRoadId " << traciVehicle->getRoadId() << std::endl;
	}
	...
	if (testNumber == testCounter++) {
		...
		if (t == 30) {
			...
			assertTrue("(Vehicle::changeRoute, 9999) vehicle avoided 28", visitedEdges.find("28") == visitedEdges.end());
